...
from motivic_cohomology import motivic_regulator
...
def run_for_window(returns, window_days):
    ...
    try:
        raw_scores = motivic_regulator(
            ret_window,
            weight=config.REGULATOR_WEIGHT,
            use_bloch=config.USE_BLOCH_TERM
        )
    ...
    top_etfs = [{"ticker": t, "motivic_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "regulator_weight": config.REGULATOR_WEIGHT,
        "use_bloch_term": config.USE_BLOCH_TERM,
        "universes": {}
    }
    ...
    out_file = f"output/motivic_cohomology_{timestamp}.json"
