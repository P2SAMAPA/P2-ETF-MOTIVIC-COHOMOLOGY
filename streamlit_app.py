...
st.set_page_config(page_title="Motivic Cohomology – Algebraic Geometry for ETFs", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📐 Motivic Cohomology Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Beilinson–Bloch regulator of ETF correlation matrices | Arithmetic complexity as alpha signal</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Motivic Cohomology")
...
st.sidebar.markdown(f"**Regulator weight:** {config.REGULATOR_WEIGHT}")
st.sidebar.markdown(f"**Bloch term:** {config.USE_BLOCH_TERM}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'motivic_cohomology_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_mode="best", selected_window=None):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Regulator score: {etf['motivic_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Regulator"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Regulator", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📐 Top ETFs by Motivic Regulator (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Motivic cohomology** is a deep algebraic invariant of algebraic varieties.
        - We interpret the ETF correlation matrix as the “motive” of the market.
        - The **Beilinson–Bloch regulator** maps motivic cohomology to real numbers via eigenvalues and eigenvectors.
        - The per‑ETF score = ∏_j λ_j^{weight·v_{ij}^2} (the product of eigenvalues weighted by the squared eigenvector components).
        - A high regulator indicates that the ETF is “arithmetically complex” – it has high‑order algebraic structure that may be predictive.
        - The best window is automatically selected as the one with the largest absolute raw regulator.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the motivic regulator per ETF.")
    for universe_name, uni_data in data["universes"].items():
...
st.sidebar.markdown("---")
st.sidebar.caption("Motivic Cohomology | Beilinson–Bloch regulator of ETF correlation spectra")
